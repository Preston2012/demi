...
ENV HUSKY=0
RUN apt-get update && apt-get install -y python3 make g++ && npm ci --omit=dev --ignore-scripts && npm rebuild better-sqlite3 && npm rebuild better-sqlite3-multiple-ciphers && npm rebuild sharp && npm cache clean --force

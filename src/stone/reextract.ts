 ...
 */
export function buildReextractionPrompt(query: string, messages: ConversationMessage[], maxMessages = 50): string {
 ...
 */
function findRelevantConversations(stone: StoneStore, query: string, maxConversations = 3): string[] {
...
/**
 * Execute re-extraction. Calls the answer model with STONE context
 * and a query-focused extraction prompt. Returns newly-extracted facts
 * ready for injection. Does NOT write to memory store, caller decides
 * whether to promote extracted facts to persistent memory.
 *
 * Flag-gated: no-op unless REEXTRACT_ENABLED=true.
 */
export async function executeReextraction(
  query: string,
  store: StoneStore,
  callLLM: (prompt: string) => Promise<string>,
  maxConversations = 3,
): Promise<Array<{ subject: string; claim: string }>> {
  if (process.env.REEXTRACT_ENABLED !== 'true') return [];

  const requests = prepareReextractions(store, query, maxConversations);
  if (requests.length === 0) {
    log.info({ query: query.substring(0, 80) }, 'Re-extraction: no relevant STONE conversations');
    return [];
  }

  const results: Array<{ subject: string; claim: string }> = [];
  for (const req of requests) {
    try {
      const response = await callLLM(req.prompt);
      const extracted = parseExtractionResponse(response);
      results.push(...extracted);
    } catch (err) {
      log.warn({ err, query: query.substring(0, 80) }, 'Re-extraction LLM call failed, continuing');
    }
  }
...
function prepareReextractions(stone: StoneStore, query: string, maxConversations = 3): ReextractionRequest[] {

...
export async function entitySplitRetrieval(
  ...
  for (const entity of entities) {
    const results = await searchVector(repo, entity, limit, undefined, nowIso);
    allResults.push(...results);
  }

  if (entities.length === 2) {
    const pairQuery = entities[0] + ' ' + entities[1];
    const pairResults = await searchVector(repo, pairQuery, limit, undefined, nowIso);
    allResults.push(...pairResults);
  }

  log.info({ entityCount: entities.length, resultsFound: allResults.length }, 'Entity-split: search complete');

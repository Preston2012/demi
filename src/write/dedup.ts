 ...
 */
async function checkExactDuplicate(repo: IMemoryRepository, sourceHash: string, userId: string): Promise<DedupResult> {
  const existing = await repo.findBySourceHash(sourceHash, userId);
  if (existing) {
    return {
      isDuplicate: true,
      matchType: 'exact',
...
async function checkSemanticDuplicate(
  ...
  const similar = await repo.findSimilar(embedding, threshold, userId);

  if (similar.length > 0) {
    // Take the most similar
    const best = similar.sort((a, b) => b.vectorScore - a.vectorScore)[0]!;
...
export async function checkDuplicate(
  ...
  // Layer 1: exact match (per-user; identical claims across users are not duplicates)
  const sourceHash = computeSourceHash(claim);
  const exactResult = await checkExactDuplicate(repo, sourceHash, userId);
  if (exactResult.isDuplicate) {
    recordDecision({
      decision_type: 'dedup_check',
      branch_taken: 'duplicate',
      outcome: 'exact',
      confidence: exactResult.similarity ?? undefined,
    });
    return exactResult;
  }
  ...
  if (embedding) {
    const semantic = await checkSemanticDuplicate(repo, embedding, threshold, userId);
    recordDecision({
      decision_type: 'dedup_check',
      branch_taken: semantic.isDuplicate ? 'duplicate' : 'unique',
      outcome: semantic.matchType ?? 'unique',
      confidence: semantic.similarity ?? undefined,
    });
    return semantic;
  }

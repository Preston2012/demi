...
async function callEvaluator(prompt: string, config: EvaluatorConfig): Promise<EvaluatorResult> {
  ...
  try {
    ...
    const parsed = parseVote(response, { provider: config.provider, model: config.model });
    ...
    const latencyMs = performance.now() - start;
    // S55: downgraded to WARN, fail-secure path returns quarantine vote, recoverable.
    log.warn({ err, provider: config.provider, model: config.model }, 'Evaluator call failed');
...
async function callPromotionEvaluator(prompt: string, config: EvaluatorConfig): Promise<PromotionEvaluatorResult> {
  ...
  try {
    const response = await callProvider(prompt, config, controller.signal);
    const latencyMs = performance.now() - start;
    const parsed = parsePromotionVote(response, { provider: config.provider, model: config.model });
    ...
    const latencyMs = performance.now() - start;
    // S55: downgraded to WARN, fail-secure path returns keep_provisional, recoverable.
    log.warn({ err, provider: config.provider, model: config.model }, 'Promotion evaluator failed');
...
function parseVote(
...
): { vote: ConsensusVote; reasoning: string } {
  ...
  try {
    ...
    // S55: include provider/model so we can identify which evaluator returns prose.
    log.warn(
      { responseSnippet: response.substring(0, 100), ...context },
      'Non-JSON consensus response, using substring fallback',
    );
...
function parsePromotionVote(
  response: string,
  context?: { provider: string; model: string },
): { vote: PromotionVote; reasoning: string } {
  const cleaned = response
    .replace(/```json\s*/g, '')
    .replace(/```\s*/g, '')
    .trim();
...
//
// S65 M13: consensus path now routes through src/llm/client.ts (engine
// callLLM) so it picks up the unified provider abstraction:
//   - all 6 providers (OpenAI, Anthropic, Google, xAI, Mistral, DeepSeek)
//     supported via model-name prefix routing
//   - prompt_cache_key on OpenAI; auto-cache observability on Mistral / DeepSeek
//   - retry + provider fallback chain on transient failure
//   - cost telemetry via getLLMCacheStats()
//
// EvaluatorConfig.apiKeys is preserved on the type for back-compat with
// existing callers (src/write/index.ts buildEvaluatorConfigs and tests),
// but is no longer read on the call path, the engine client reads provider
// keys from process.env directly.

async function callProvider(prompt: string, config: EvaluatorConfig, signal?: AbortSignal): Promise<string> {
  void config.apiKeys; // S65 M13: unused on the engine path
  // System message must reinforce the user prompt's JSON contract.
  // buildEvalPrompt and buildPromotionPrompt both demand a single JSON
  // object {vote, reasoning}, and parseVote / parsePromotionVote both
  // JSON.parse() the response. A system prompt instructing single-line
  // text output would silently regress every consensus call to the
  // substring fallback in parseVote (which is also a known prompt
  // injection target per the C4 comment in that function). Keep this
  // tight and JSON-only.
  const consensusSystem =
    'You are a write-gate evaluator. Output exactly one JSON object as instructed by the user prompt, no preamble, no markdown fences, no commentary. Both "vote" and "reasoning" fields are required.';
  return await callLLM(config.model, consensusSystem, prompt, 200, 0, {
    cacheKey: `demiurge:consensus:${config.provider}:v1`,
    signal,
  });
}
...
export async function runConsensus(
  ...
  for (const result of settled) {
    if (result.status === 'fulfilled') {
      ...
      // S55: downgraded to WARN, Promise.allSettled rejection is fail-secure (vote excluded from tally, quarantine fallback covers low-quorum case).
      log.warn({ err: result.reason }, 'Evaluator promise rejected');
    }
  }
  ...
  if (votes.length < minAgreement) {
    ...
    recordDecision({
      decision_type: 'consensus_aggregate',
      branch_taken: 'quarantine',
      outcome: 'low_quorum',
      inputs: { evaluator_count: evaluators.length, responded_count: votes.length },
    });
    return {
      decision: 'quarantine',
      votes,
  ...
  recordDecision({
    ...
    outcome: unanimous ? 'unanimous' : 'majority',
    inputs: { evaluator_count: evaluators.length, responded_count: votes.length },
  });
...
export async function runPromotionConsensus(
  ...
  for (const result of settled) {
    if (result.status === 'fulfilled') {
      ...
      // S55: downgraded to WARN, same fail-secure reasoning as above.
      log.warn({ err: result.reason }, 'Promotion evaluator promise rejected');
    }
  }

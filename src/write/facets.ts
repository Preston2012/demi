...
const KIND_PATTERNS: [RegExp, string][] = [
  [
    /\b(married|divorced|dating|engaged|sibling|parent|child|friend|partner|spouse|boyfriend|girlfriend|husband|wife|mother|father|sister|brother|son|daughter|roommate|colleague|boss|mentor)\b/i,
    'relationship',
  ],
  [
    /\b(born|age|name is|goes by|identifies as|nationality|ethnicity|gender|pronouns|lives in|moved to|from|grew up|raised in|hometown|citizen)\b/i,
    'identity',
  ],
  [
    /\b(wants to|plans to|planning|goal|aspir|dream|intend|aim|hoping to|working toward|target|objective|ambition)\b/i,
    'goal',
  ],
  [
    /\b(prefer|favorite|loves|hates|enjoys|dislikes|likes|fan of|into|passionate about|obsessed with|allergic|vegetarian|vegan|doesn't eat|avoids|can't stand)\b/i,
    'preference',
  ],
  [
    /\b(started|began|finished|completed|graduated|promoted|hired|fired|quit|joined|left|moved|bought|sold|adopted|lost|won|earned|received|diagnosed|recovered|launched|released|published|performed)\b/i,
    'event',
  ],
  [
    /\b(works at|job|career|occupation|profession|employed|position|role|title|salary|income|studying|student|enrolled|major|degree)\b/i,
    'status',
  ],
  [
    /\b(thinks|believes|feels|opinion|view|perspective|stance|attitude|worried|concerned|excited|anxious|happy|sad|frustrated|confident)\b/i,
    'opinion',
  ],
];
...
function extractSlotKey(claim: string, factKind: string): string | null {
  ...
  if (factKind === 'relationship') {
    ...
    const relMatch = claim.match(
      /\b(married|dating|engaged|sibling|parent|child|friend|partner|spouse|boyfriend|girlfriend|husband|wife|mother|father|sister|brother|roommate|colleague|boss|mentor)\b/i,
    );
...
/**
 * Packet A: denormalize facet subjects into entity_index for O(1) entity-boost lookups.
 * userId defaults to 'system' (no user-scoping in current write pipeline).
 */
function populateEntityIndex(db: Database.Database, factId: string, facet: FactFacet, userId: string = 'system'): void {
  const now = new Date().toISOString();
  const insert = db.prepare(
    `INSERT OR IGNORE INTO entity_index (entity_text, memory_id, is_primary, user_id, created_at) VALUES (?, ?, ?, ?, ?)`,
  );
  const primary = facet.primary_subject?.trim().toLowerCase();
  if (primary) {
    insert.run(primary, factId, 1, userId, now);
  }
  try {
    const mentioned: unknown = JSON.parse(facet.mentioned_subjects || '[]');
    if (Array.isArray(mentioned)) {
      for (const m of mentioned) {
        if (typeof m !== 'string') continue;
        const t = m.trim().toLowerCase();
        if (!t || t === primary) continue;
        insert.run(t, factId, 0, userId, now);
      }
    }
  } catch {
    // mentioned_subjects malformed JSON, non-critical, skip silently
  }
}
...
export function populateFacets(db: Database.Database, record: MemoryRecord): void {
  try {
    ...
    populateEntityIndex(db, record.id, facet);
...
export function getFacet(db: Database.Database, factId: string): FactFacet | null {
  const row = db.prepare('SELECT * FROM fact_facets WHERE fact_id = ?').get(factId) as
...
export function getFacetsBySubject(db: Database.Database, subject: string): FactFacet[] {
  const rows = db.prepare('SELECT * FROM fact_facets WHERE primary_subject = ?').all(subject) as Record<
    string,
    unknown
  >[];
  return rows.map((row) => ({

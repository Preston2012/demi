...
async function generateTitleAndSummary(
  ...
  // S65 Sprint 1: Anthropic-specific apiKey gate replaced with
  // provider-agnostic check. If no LLM provider keys are configured at all,
  // skip the LLM call and go straight to deterministic fallback. Otherwise
  // try the engine callLLM (gpt-4o-mini default) and let the try/catch
  // below handle transient failures.
  void apiKey;
  ...
  const model = process.env.EPISODE_TITLE_MODEL || 'gpt-4o-mini';

  // S71 T4: persistent cache check. The in-process LRU on callLLM helps
  // within a single bench run, but LME fires this hook 40-60x per question
  // (vs ~6x per conversation for BEAM/LOCOMO) so every fresh run paid the
  // full cost cold. Per brain #2215, key on (subject, sorted-claims, model).
  let cache: ReturnType<typeof getSharedCache> | null = null;
  try {
    cache = getSharedCache();
    const hit = cache.getEpisodeTitle(subject, claims, model);
    if (hit) {
      return { title: hit.title, summary: hit.summary };
    }
  } catch (cacheErr) {
    // Cache unavailable (test env, disk failure, etc.), fall through to LLM call.
    // cache remains null from declaration; outer code checks via 'if (cache && ...)'.
    log.debug({ err: cacheErr }, 'Episode title cache unavailable, falling through');
  }
  ...
  try {
    ...
    // S65 M13: engine callLLM, gpt-4o-mini default for episode title gen.
    const text = (
      await callLLM(
        model,
        'You write episode titles and summaries in the exact format requested by the user prompt. Reply with both TITLE: and SUMMARY: lines.',
        prompt,
        120,
        0,
        { cacheKey: 'demiurge:episodes:title:v1' },
      )
    ).trim();
    ...
    const title = titleMatch?.[1]?.trim().slice(0, 100) || deterministicTitle(subject, claims);
    const summary = summaryMatch?.[1]?.trim().slice(0, 200) || deterministicSummary(subject, claims);

    // S71 T4: write through to persistent cache on success. Fallback path
    // (deterministic) is NOT cached so a transient LLM failure doesn't
    // poison the cache with deterministic content.
    if (cache && titleMatch && summaryMatch) {
      try {
        cache.putEpisodeTitle(subject, claims, model, title, summary, 0);
      } catch (writeErr) {
        log.debug({ err: writeErr }, 'Episode title cache write failed');
      }
    }
...
function insertEpisode(db: Database.Database, episode: Episode): void {
  db.prepare(
    `
    INSERT OR REPLACE INTO episodes (id, user_id, subject, title, summary, timeframe_start, timeframe_end, session_source, fact_count, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `,
  ).run(
    episode.id,
...
export async function buildEpisodes(
  ...
  if (!onByDefault(process.env.EPISODES_ENABLED)) return [];
  if (factIds.length < MIN_CLUSTER_SIZE) return [];

  // Load facts and facets
  const facts: FactWithFacet[] = [];
  for (const fid of factIds) {
    const row = db.prepare('SELECT * FROM memories WHERE id = ? AND deleted_at IS NULL').get(fid) as
      | Record<string, unknown>
      | undefined;
    if (!row) continue;
    const facet = db.prepare('SELECT * FROM fact_facets WHERE fact_id = ?').get(fid) as
      | Record<string, unknown>
      | undefined;
    if (!facet) continue;

    // Minimal record reconstruction for embedding lookup
    const embedding = db.prepare('SELECT embedding FROM memories_vec WHERE id = ?').get(fid) as

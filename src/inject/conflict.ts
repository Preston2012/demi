...
import { recordConflict } from '../telemetry/index.js';
 ...
 */
export function detectExplicitConflicts(candidates: FinalScoredCandidate[]): ConflictNotice[] {
 ...
 */
export function detectSubjectConflicts(candidates: FinalScoredCandidate[]): ConflictNotice[] {
  const notices: ConflictNotice[] = [];
  ...
  for (const [subject, group] of bySubject) {
    ...
    const uniqueClaims = new Set(group.map((c) => c.candidate.record.claim.toLowerCase().trim()));
...
function buildConflictMessage(
  ...
  const conflictDate = new Date(conflicting.candidate.record.updatedAt).toLocaleDateString();
 ...
 */
export function detectAllConflicts(candidates: FinalScoredCandidate[]): ConflictNotice[] {
  ...
  const merged = deduplicateNotices([...explicit, ...subject]);
  if (merged.length > 0) {
    recordConflict({
      member_ids: merged.flatMap((n) => [n.memoryId, n.conflictsWithId]),
      resolution: 'surfaced',
      surfaced: true,
    });
  }
 ...
 */
export function buildConflictTagMap(candidates: FinalScoredCandidate[]): Map<string, string> {

...
async function judgeBeamAnswer(
  ...
  hypothesis: string,
  apiKey: string,
  // S65 M13: default judge swapped from claude-haiku to gpt-4o-mini.
  // Anthropic stays available via callLLM fallback chain only.
...
Output ONLY the number: 0, 0.5, or 1`;
  ...
  // S65 M13: route through engine callLLM. Anthropic still callable here
  // as a fallback if model='claude-...' is passed explicitly, but default
  // judge is gpt-4o-mini for cost. apiKey arg is preserved for back-compat
  // with existing callers but unused on the engine path (callLLM reads
  // provider keys from env).
  void apiKey;
  const judgeOut = await callLLM(
    model,
    'You are a strict scoring judge. Output only the requested numeric score.',
    prompt,
    10,
    0,
    { cacheKey: 'demiurge:beam:judge:v1' },
  );
  // S65 prompt-audit pass 2: parser was strict-equal on '1' / '1.0' / '0.5'
  // and silently bucketed everything else to 0.0 (wrong). Models routinely
  // emit '0.50', '1 ', 'Score: 0.5', or trailing punctuation. Now extracts
  // the first numeric token and matches with tolerance.
  const text = (judgeOut ?? '0').trim();
  const match = text.match(/(-?\d+(?:\.\d+)?)/);
  if (!match) return 0.0;
  const n = parseFloat(match[1]!);
  if (Number.isNaN(n)) return 0.0;
  if (n >= 0.85) return 1.0;
  if (n >= 0.35) return 0.5;
  return 0.0;
}

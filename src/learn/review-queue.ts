...
export interface ReviewQueueService {
  getPending(limit?: number, userId?: string): Promise<MemoryRecord[]>;
  decide(decision: ReviewDecision, userId?: string): Promise<void>;
...
export function createReviewQueue(repo: IMemoryRepository): ReviewQueueService {
  return {
    async getPending(limit = 50, userId = 'system'): Promise<MemoryRecord[]> {
      const results = await repo.getPendingReview(limit, userId);
      logger.debug(`Fetched ${results.length} pending reviews`);
      return results;
    },

    async decide(decision: ReviewDecision, userId = 'system'): Promise<void> {
      const { memoryId, action, reason } = decision;
      const memory = await repo.getById(memoryId, userId);
      if (!memory) {
        throw new Error(`Memory not found: ${memoryId}`);
      }

      if (memory.reviewStatus !== 'pending') {
        throw new Error(`Memory ${memoryId} is not pending review (status: ${memory.reviewStatus})`);
      }
      ...
      logger.info(`Review decision: ${action} on ${memoryId}${reason ? ` (${reason})` : ''}`);
    },

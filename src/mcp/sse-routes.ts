 ...
 */
export function registerSseRoutes(app: FastifyInstance, dispatch: CoreDispatch): SseSessionManager {
  ...
  app.get('/sse', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const server = createMcpServer(dispatch, req.principal?.userId ?? 'system');
  ...
  app.post('/messages', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    await session.transport.handlePostMessage(req.raw as IncomingMessage, reply.raw, req.body);
  });
